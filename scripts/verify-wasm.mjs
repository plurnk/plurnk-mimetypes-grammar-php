#!/usr/bin/env node
// CI byte-identical reproducibility check for php.wasm.
...
const committed = await readFile(path.join(repoRoot, "php.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed php.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-php-verify-"));
...
console.log(`rebuilt php.wasm sha256: ${rebuiltHash}`);
