#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-php.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-php-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter/tree-sitter-php.git", "src"], { cwd: work });
...
const buildCwd = path.join(work, "src", "php");
await run(cli, ["generate"], { cwd: buildCwd });
await run(cli, ["build", "--wasm"], { cwd: buildCwd });

// Locate produced wasm and copy as php.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "php.wasm"));
const bytes = (await readFile(path.join(repoRoot, "php.wasm"))).length;
console.log(`php.wasm: ${bytes} bytes (built from ${pin})`);
